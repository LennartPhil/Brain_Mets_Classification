...
path_to_weights = path_to_logs + "/transfer_vit_00_2_cls_cutout_rgb_lr_run_2024_10_22_22_12_28/fold_0/saved_weights.weights.h5"
...
def train_ai():
    ...
    if use_k_fold:
        ...
        for fold in range(10):
            ...
            # build model
            model = build_transfer_vit_model(clinical_data = clinical_data, use_layer = use_layer)
        ...
        model = build_transfer_vit_model(clinical_data = clinical_data, use_layer = use_layer, trainable = False)
...
def build_transfer_vit_model(clinical_data, use_layer, trainable = True):
    ...
    x = hub.KerasLayer("https://www.kaggle.com/models/spsayakpaul/vision-transformer/TensorFlow2/vit-b16-fe/1", trainable=trainable)(batch_normed_augment)
    #x = tf.keras.layers.GlobalMaxPool2D()(x)

    vit = tf.keras.layers.Flatten()(x)
    ...
    model = tf.keras.Model(inputs = [image_input, sex_input, age_input, layer_input], outputs = [output], name = "transfer_vit_model")
