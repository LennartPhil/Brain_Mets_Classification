...
# - add z score normalization before any registration and brain extraction




def run_preprocessing_old():
    ...
    path_to_preprocessed_files = f"{path_to_output}/preprocessed_n4_correction_brainlesion_{timeFormatted}"

    preprocessed_folder_exists = False

    for file in os.listdir(path_to_output):
        if "n4_correction_brainlesion" in file:
...
def run_preprocessing():
    ...
    for patient in tqdm(patients):
        ...
        if patient.name not in patients_to_preprocesse:
            ...
            continue
        print("processing: ", patient.name)
        preprocessed_folder = [folder for folder in patient.dirs() if folder.name == "n4-z-score-normalized"][0]
...
def preprocess_exam_in_brats_style(inputDir: str, patID: str, outputDir: str) -> None:
    ...
    t1_file = inputDir / patID + "_n4_z_score_normalized_" + "T1w.nii.gz"
    t1c_file = inputDir / patID + "_n4_z_score_normalized_" + "T1c.nii.gz"
    t2_file = inputDir / patID + "_n4_z_score_normalized_" + "T2w.nii.gz"
    flair_file = inputDir / patID + "_n4_z_score_normalized_" + "FLAIR.nii.gz"
...
def normalize():
    ...
    path_to_raw = Path("/Users/LennartPhilipp/Desktop/testing_data/raw_data")
    ...
    patients = [patient for patient in os.listdir(path_to_raw) if os.path.isdir(os.path.join(path_to_raw, patient))]
    print(f"found {len(patients)} patients")

    for patient in tqdm(patients):
        print(f"currently normalizing patient {patient}")

        path_to_patient_files = Path(path_to_raw) / Path(patient) / Path("anat")

        raw_files = [file for file in os.listdir(path_to_patient_files) if ".nii.gz" in file]

        # create subfolder for each patient
        path_to_normalized_folder = Path(path_to_preprocessed_files) / Path(patient) / "z_score_n4_brainlesion_percentile"
        os.makedirs(path_to_normalized_folder, exist_ok=True)

        for raw_file in raw_files:

            sequence_type = raw_file.split("_")[1].split(".nii")[0]
            file_name = patient + "z_score_n4_brainlesion_percentile" + sequence_type + ".nii.gz"
            ...
            n4_bias_correction(z_score_normalized_image, path_to_normalized_file)
...
def n4_bias_correction(image, path_to_output):
    """
    Perform N4 bias field correction on an image using a mask.

    Args:
        path_to_image (str or Path): The path to the input image.
        path_to_mask (str or Path): The path to the mask image.
        path_to_output (str or Path): The path to save the corrected image.
